...
{
    glm::vec3 pos;
    glm::vec3 color; // TODO: @MaxCompleteAPI, this is probably unecessary
    glm::vec2 texCoord;
    ...
    static void getAttributeDescriptions(uint32_t binding, std::vector<VkVertexInputAttributeDescription> &attributeDescriptions)
    {
        ...
        attributeDescriptions.push_back({});
        attributeDescriptions[index].binding  = binding;
        attributeDescriptions[index].location = 1;
        attributeDescriptions[index].format   = VK_FORMAT_R32G32B32_SFLOAT;
        attributeDescriptions[index].offset   = offsetof(Vertex, color);

        index = attributeDescriptions.size();
        attributeDescriptions.push_back({});
        attributeDescriptions[index].binding  = binding;
        attributeDescriptions[index].location = 2;
